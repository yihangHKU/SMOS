function L = Fmdl(imat, prm, st, frame)

% model background
% output: purified integrated points

%% purify integraed voxels
ref           = FJloa(st, frame);                         % use current coordinate as a reference coordinate 
for frm       = frame : -1 : frame - st.rd.dl + 1         % go over previous frames [number of frames to process]
if  frm       > 0                                         % minimum start farme is 1 
    
pnt           = FJloa(st, frm);                           % load points
pnt           = FJtns(pnt, ref);                          % transform to the current coordinate
pnt           = FJcrp(pnt, st, frame);                    % crop points to the inside the local grid and image
[~, lmat]     = Fvox(prm, st, pnt.pts);                   % remove ground and voxelize points    

idx.occ       = (imat.occ - st.fr.frg*lmat.occ) > 0;              % remove dynamic cells from integrated grid
imat.occ      = (idx.occ) .* imat.occ;

end
end
%% vectorize
[I, J, K]     = ind2sub(size(imat.occ), find(imat.occ)); ipts.uni = [I, J, K]; % unique indexes 
ipts.unq      = [st.vx.x * (I - 1) + st.vm.xb, st.vx.y * (J - 1) + ...         % unique start point locations
...
function pts    = FJtns(pts, ref)                                                % previous and reference(current)
...
temp(:, 1:3)    = pts.ptn(:, 1:3) - repmat(ref.trn', size(pts.pts, 1), 1);     % translation
pts.pts(:, 1:3) = (ref.rtn \ temp(:, 1:3)')';                                  % rotation

end

function hst    = FJcrp(hst, st, frame)
